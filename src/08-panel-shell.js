...
function scaffoldSections(){
  ...
  <div class="tabstack" id="combatStack">
    ...
    <div class="card stagecard" id="combatStageCard">
      ...
      <div class="row" style="justify-content:space-between;margin-top:4px">
        ...
        <span class="hint">点上方「👥 添加角色」加入 / 移出；头像上显示 HP/SAN/MP</span>
    ...
    <div class="card" id="combatMemberCard">
      <div class="panel-head"><b>🪖 战斗成员</b><span class="hint">点一行看详情；左边「加入/移出」在「👥 添加角色」里</span></div>
  ...
  <div class="modal-body" style="font-size:13px">
    ...
    <h4>✏️ 编辑调查员</h4><p>点小卡右下角 ✏️：属性、护甲（可挑参考防具预置）、HP/SAN/MP、技能（高于基础值的算「已加点」排上面，其余收进折叠区）、武器、背包 / 随身用品、剧情道具、法术、资产、背景、任意特长（每行一条，写回卡右上角那几格）、经历都在这里改。武器「类型」可从卡里「武器列表」下拉选，选完技能 / 伤害 / 射程 / 弹匣会自动填好；改「信用评级」时技能里的「信用评级」成功率会跟着一起改。</p>
    <h4>⬇ 导出人物卡</h4><p>小卡右下角 ⬇ 导出卡：先按“最初导入那张卡”算出这次团的变化，写进卡里的「调查员经历」（模组名用顶部「🎪 本次团名」，技能变化写进「成长」列），再把整张卡填进<b>你导入时用的那张模板</b>下载，文件名是 团名-角色名.xlsx。卡里原本的公式都会保留：武器“选类型自动算”、成功率按使用技能取值（查不到该技能时保留公式 + 原卡数值，不留空、不乱码）、信用评级查表、本职★ 查表、<b>属性雷达图</b>；自定义子技能名按卡里「类别 + 具体名」两格分段写回；中式职业扩展那张卡的本职技能按它自己的职业表算。</p>
    <h4>📊 属性雷达图</h4><p>点小卡右下角 ✏️ 打开详情，属性 3×3 右边就是雷达图（力量 / 体质 / 体型 / 敏捷 / 外貌 / 智力 / 意志 / 教育 / 幸运），和卡里「附表」那张九维图同一套数，改属性会实时重画。</p>
    <h4>📖 模组（右半屏）</h4><p>菜单栏点「📖 模组」：左边照常带团，右半边用来放模组。支持 Word（.docx）、PDF、txt / md，直接拖进去就行。Word 会连着表格 / 图片 / 标题一起排成网页看（还能调字号），上方搜索框回车可高亮命中、↑ ↓ 逐条跳转；PDF 用浏览器自带阅读器（可缩放、可搜）。文件存在本机浏览器里（刷新后还在），不会上传。中间的细条可拖动调整左右宽度，双击回一半一半。</p>
    <h4>📚 规则书（右半屏）</h4><p>菜单栏点「📚 规则书」：整套 COC7 核心规则书（318 页）按<b>原版 PDF</b>放在右半屏，表格 / 颜色 / 流程图和纸书一模一样，正文里 PDF 自带的目录 / 页码 / Ctrl+F 都能用；左边还有 <b>书签目录</b>（点条目直接跳页）和 <b>全文搜索</b>（输入关键词回车，命中页会列出来，点一下就跳过去），目录可以「☰ 收起目录 / 展开目录」。</p>
    ...
    <h4>🗺️ 地图与路线</h4><p>上面一条二级菜单栏：🗺 地图（切地图 / 载入预设 / 缩放 / 比例尺 / 换底图）· 🧍 地图角色 · 🧩 摆件素材 · 🚗 载具·时间速度（点开就用，再点收起，一次只开一个），下面整幅地图占满宽度，再往下是路线时间计算 / 地点 / 道路路径（一张地图都没有时也还在，会提示你「＋ 新建地图」）。三个工具：🖱 拖动地点（相连道路的 km 实时重算）· 🧍 移动角色/摆件（只挪角色和素材，不动地点与道路）· 📍 添加地点。比例尺「1格 = __ km」→ 设；切换预设地图时里程已按当前比例尺算好。</p>
