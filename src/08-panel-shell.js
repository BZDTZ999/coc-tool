...
function scaffoldSections(){
  ...
  <div class="modal-body" style="font-size:13px">
    ...
    <h4>📖 模组（右半屏）</h4><p>菜单栏点「📖 模组」：左边照常带团，右半边用来放模组。支持 <b>PDF</b>、<b>Word（.docx）</b>、<b>图片</b>（png / jpg / gif / webp…）、<b>txt / md</b>。可以「⬆ 添加文件」一次选多个、点「📁 文件夹」整个文件夹加进来，或者直接把文件 / 文件夹<b>拖到窗口里任何地方</b>（会自动打开模组栏，拖拽时顶上提示「松手就把这些文件放进模组」）；加进去的会在顶上排成一排<b>标签页</b>，点一下换一份，标签上的 ✕ 把那份移出去，多文件模组不用再来回换。Word 会连着表格 / 图片 / 标题一起排成网页看（还能调字号），上方搜索框回车可高亮命中、↑ ↓ 逐条跳转；PDF 用浏览器自带阅读器（可缩放、可搜）；图片点一下在「适应窗口 / 原始大小」之间切。文件存在本机浏览器里（刷新后还在），不会上传。中间的细条可拖动调整左右宽度，双击回一半一半。</p>
