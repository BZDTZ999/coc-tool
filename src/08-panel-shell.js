...
function scaffoldSections(){
  ...
  <div class="srvcols">
    ...
    <div class="srvmain">
      <div class="card" id="importCard">
        ...
        <input type="file" id="fileImport" accept=".xlsx,.xls" class="file-hidden" onchange="onPickCardFile(event)">
  ...
  <div class="tabstack" id="mapStack">
    ...
    <div class="podrow" id="mapPods">
      <div class="pod card" id="pod-cards" hidden>
        ...
        <div class="row mapctrlrow" style="justify-content:space-between;flex-wrap:wrap;gap:8px">
          ...
          <div class="row" style="gap:6px;flex-wrap:wrap">
            ...
            <label class="muted" style="flex-direction:row;align-items:center;gap:4px">底图<input type="file" id="mapBg" accept="image/*" class="file-hidden" onchange="onBgPick(event)"><button class="small ghost" onclick="document.getElementById('mapBg').click()">上传</button></label>
          </div>
        </div>
      </div>
      ...
      <div class="pod card" id="pod-props" hidden>
        ...
        <input type="file" id="customPropFile" accept="image/*" multiple class="file-hidden" onchange="addCustomPropFiles(event)">
      </div>
  ...
  <div class="tabstack" id="combatStack">
    ...
    <div class="card stagecard" id="combatStageCard">
      <div class="panel-head"><b>🏟 战斗场景</b>
        <div class="row">
          <label class="muted" style="flex-direction:row;gap:4px">上传底图<input type="file" id="battleBg" accept="image/*" class="file-hidden" onchange="onBattleBg(event)"></label>
      ...
      <details class="ccard" data-coll="bprops" id="battlePropsCard">
        ...
        <input type="file" id="battlePropFile" accept="image/*" multiple class="file-hidden" onchange="onBattlePropFile(event)">
  ...
  <div class="modal-body" style="font-size:13px">
    ...
    <h4>📖 模组（右半屏）</h4><p>菜单栏点「📖 模组」：左边照常带团，右半边用来放模组。支持 <b>PDF</b>、<b>Word（.docx）</b>、<b>图片</b>（png / jpg / gif / webp…）、<b>txt / md</b>。可以「⬆ 添加文件」一次选多个、点「📁 文件夹」整个文件夹加进来，或者直接把文件 / 文件夹<b>拖到窗口里任何地方</b>（会自动打开模组栏，拖拽时顶上提示「松手就把这些文件放进模组」）；加进去的会在顶上排成一排<b>标签页</b>，点一下换一份，标签上的 ✕ 把那份移出去，多文件模组不用再来回换；<b>标签可以按住拖动换顺序</b>（顺序会记住），一行放不下时标签名会自动缩短（全称 → 省略号 → 只剩两个字），这一条的高度是固定的，不会被挤扁。手机上点「⬆ 添加文件」从「文件」App 里挑（可以多选；Safari 不支持选目录，所以手机不显示「📁 文件夹」）。Word 会连着表格 / 图片 / 标题一起排成网页看（还能调字号），上方搜索框回车可高亮命中、↑ ↓ 逐条跳转；PDF 用<b>工具自带的阅读器</b>看：<b>整本一页接一页连着排，手机 · 平板上上下滑动就是翻页</b>（电脑上滚轮 / 拖动一样顺），页码跟着你正看的那一页走；「☰ 跳页」能按<b>书签目录或整本页码方格</b>快速翻（没有目录的 PDF 也能按页码跳），另有上一页 / 下一页 / 输页码 / A＋A－ 缩放 / 适宽，手机 · 平板上还能双指捏合；点「↗ 新窗口」会交给系统阅读器）；图片点一下在「适应窗口 / 原始大小」之间切。文件存在本机浏览器里（刷新后还在），不会上传。中间的细条可拖动调整左右宽度，双击回一半一半。</p>
    <h4>📚 规则书（右半屏）</h4><p>菜单栏点「📚 规则书」：整套 COC7 核心规则书按<b>原版 PDF</b>放在右半屏，表格 / 颜色 / 流程图和纸书一模一样；左边有 <b>书签目录</b>（点条目直接跳页）和 <b>全文搜索</b>（输入关键词回车，命中页会列出来，点一下就跳过去），目录可以「☰ 收起目录 / 展开目录」。正文<b>一页接一页连着排</b>，手机 · 平板上<b>上下滑动就是翻页</b>（电脑上滚轮 / 拖动一样顺），工具条页码跟着你正看的那一页走；「☰ 跳页」是快速翻页面板：PDF 自带书签就先列书签目录，没有书签就给整本页码方格，点一下就跳过去。翻页也能用「‹ 上一页 / 下一页 ›」或直接输页码，缩放用 A－ / A＋ / 适宽（手机 · 平板上也能双指捏合），点目录或搜索命中都会真的翻到那一页。</p>
    <h4>🧰 更多小玩意儿（右半屏）</h4><p>菜单栏点「🧰 更多小玩意儿」：左边照常带团，右半边三个小工具（顶上子页签切换）。<b>🎲 跑团随机</b>：随机名字（中文 / 英文带中文翻译 / 日文带假名读音，可选性别，一次 1/6/12/20 个）、随机 NPC（姓名 / 年龄 / 性别 / 职业 / 性格 / 外貌 / 着装 / 小习惯 / 家庭状况 / 爱好 / 恐惧 / 秘密 / 信仰 / 口头禅一次全出，某一行不满意就点那一行的 ⟳ 单独重掷）、随机地点（场所 / 城镇 / 街道 / 建筑 / 房间，每处带一句能直接用的话）；结果点一下就能复制。<b>🌙 天文·天象</b>：选<b>日期</b> + <b>地点</b>（内置 100 来个城市，也能手填经纬度与时区），给出当天的月相（名称 / 月龄 / 照明率）、月出月落、日出日落、白昼黑夜时长、天黑与完全黑暗的时段、今晚能看到的亮星与星座、肉眼可见的行星（带方位与星等）、流星雨（峰值前后，含月光影响）、当地可见的日食月食，还有折叠的<b>本月天象日历</b>（月相节点 / 流星雨峰值 / 日月食 / 节气）；极昼极夜白夜会照实说明，时间按该城市标准时算（不换算夏令时）。天文数据用的是真实星历（astronomy-engine，MIT），不是编的。<b>✍️ KP正在瞎编！</b>：纯本地、不联网的描写助手 —— 输入一句话（例：「凌晨三点，外面下着瓢泼大雨，废弃病院里特别阴冷」）或点几个标签，就出一段能直接念的描写；场景 / 时间 / 天气 / 氛围可以叠加，说「不要出现怪物」会当成禁止条件（只说「不要马上出现怪物」则按延迟处理，它会存在但这一段不露面）。另有 12 个场景的一键生成、独立页签的<b>怪物描写</b>（9 种怪物各有自己的外貌 / 动作 / 声音 / 气味 / 环境变化，不套同一套模板）、<b>我的素材</b>（自己写一句加进来，自动认标签、自己存本机、参与之后的生成）。结果可再来一段 / 继续（保留场景与已建立的事实）/ 扩写 / 选中某句锁定（重新生成时那句不变）/ 复制，另有短 / 中 / 长三档长度。素材不够时按层降级并就地提示，不会偷偷换掉你明确给的条件。</p><h4>🏷 便签标签</h4><p>调查员页最左侧可自定义标签（emoji、任意长度、可改色、可删）。点选标签再点小卡即可贴上，也能直接把标签拖到小卡上；便签可挪到别的卡，点 ✕ 移除，每张小卡最多 6 个。</p>
    ...
    <h4>⚔️ 战斗</h4><p><b>拖动场景里的角色只挪位置，不会弹详情；想开详情就双击角色</b>（全屏时点空白处收起详情）。上面一条二级菜单栏：👥 添加角色（把调查员 / NPC / 临时成员加入战斗）· ⚔️ 战斗桌（本次战斗标题 / 下一轮 / 清空，点开就用、再点收起），下面战斗场景占满宽度，再往下依次是战斗成员表、行动日志；点角色后详情浮在屏幕最左侧。成员自动左右排布（可拖动）；攻击不用先选目标，直接打对面第一个还站着的成员；改 HP/SAN/MP/护甲或实时增删武器 / 道具 / 法术都会同步回角色档案与小卡，也可点「↻ 同步档案」。状态徽标与 🎨 场景道具素材库都在这一页。</p>
    ...
    <h4>🎨 背景 · 🐙 装饰 · 📱 手机与平板</h4><p>第一次打开会从 <b>暗金 / 橄榄绿</b> 里随机挑一套当默认皮肤（不再是以前那身深蓝），挑到哪套会记住，<b>刷新不变</b>；自己选的颜色 / 上传的背景图<b>刷新后也一定还在</b>（颜色存本机，背景图存在浏览器的 IndexedDB 里，不占 localStorage 那 5MB 的额度）。想还原点「↺ 恢复默认背景」（会重新随机一套）。「🐙 装饰」那层漂浮小图标<b>默认就是开着的</b>，只有自己点「装饰：关」才关。</p><p><b>手机 / 平板</b>（iPhone 16 / 17、iPad mini 6、iPad Air 4 都调过）：顶栏第一行是「🪄 带团妙妙小工具 + 🎪 本次团名」，第二行是备份 / 背景 / 装饰 / 说明；菜单栏一行横着划。<b>「🎪 本次团名」就在品牌名那一行</b>（不再塞在菜单栏中间，免得 iPad 横屏把菜单栏挤成两行）。点「📖 模组 / 📚 规则书 / 🧰 更多小玩意儿」时右半屏会<b>铺满整屏</b>，右上角 ✕ 收起；手机上加模组点「⬆ 添加文件」从「文件」App 里挑（Safari 不支持选目录，所以手机不显示「📁 文件夹」）。看 PDF 时上方工具条<b>只留图标、永远一行</b>（‹ 2/3 › ☰ A－ A＋ 适宽 ↗），说明文字收起来、文件名只出现在标签页上，地方都留给正文。</p>
    ...
    <h4>💾 数据</h4><p>数据只存本机浏览器。请常用右上角「⬇ 备份数据」导出 JSON 定期存档；本工具不上传任何数据。「⚠️ 清空全部本地数据」会连上传的模组与背景图一起清掉。</p>
    <div class="vdiv"></div>
