...
function scaffoldSections(){
  ...
  <div class="modal-body" style="font-size:13px">
    ...
    <h4>📖 模组（右半屏）</h4><p>菜单栏点「📖 模组」：左边照常带团，右半边用来放模组。支持 <b>PDF</b>、<b>Word（.docx）</b>、<b>图片</b>（png / jpg / gif / webp…）、<b>txt / md</b>。可以「⬆ 添加文件」一次选多个、点「📁 文件夹」整个文件夹加进来，或者直接把文件 / 文件夹<b>拖到窗口里任何地方</b>（会自动打开模组栏，拖拽时顶上提示「松手就把这些文件放进模组」）；加进去的会在顶上排成一排<b>标签页</b>，点一下换一份，标签上的 ✕ 把那份移出去，多文件模组不用再来回换；<b>标签可以按住拖动换顺序</b>（顺序会记住），一行放不下时标签名会自动缩短（全称 → 省略号 → 只剩两个字），这一条的高度是固定的，不会被挤扁。手机上点「⬆ 添加文件」从「文件」App 里挑（可以多选；Safari 不支持选目录，所以手机不显示「📁 文件夹」）。Word 会连着表格 / 图片 / 标题一起排成网页看（还能调字号），上方搜索框回车可高亮命中、↑ ↓ 逐条跳转；PDF 用<b>工具自带的阅读器</b>看（上一页 / 下一页 / 输页码 / A＋A－ 缩放 / 适宽，手机 · 平板上还能双指捏合；点「↗ 新窗口」会交给系统阅读器）；图片点一下在「适应窗口 / 原始大小」之间切。文件存在本机浏览器里（刷新后还在），不会上传。中间的细条可拖动调整左右宽度，双击回一半一半。</p>
    <h4>📚 规则书（右半屏）</h4><p>菜单栏点「📚 规则书」：整套 COC7 核心规则书按<b>原版 PDF</b>放在右半屏，表格 / 颜色 / 流程图和纸书一模一样；左边有 <b>书签目录</b>（点条目直接跳页）和 <b>全文搜索</b>（输入关键词回车，命中页会列出来，点一下就跳过去），目录可以「☰ 收起目录 / 展开目录」。翻页用上方「‹ 上一页 / 下一页 ›」或直接输页码，缩放用 A－ / A＋ / 适宽（手机 · 平板上也能双指捏合），点目录或搜索命中都会真的翻到那一页。</p>
